<template>
  <div class="breadcrumbs">
    <template v-for="(crumb, index) in crumbs" :key="crumb">
      ...
      <FmIcon v-if="index !== last" :size="22" icon="mdi-chevron-right" color="var(--on-surface)" />
    </template>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const last = computed(() => (!props.crumbs.length ? 0 : props.crumbs.length - 1));

  const handleBreadcrumbClick = (clickedIndex: number) => {
    const newCrumbs =
      clickedIndex < last.value ? props.crumbs.slice(0, clickedIndex + 1) : props.crumbs;

    let newPath = cloneDeep(newCrumbs)
      .map((item) => (item.path || item.title).trim().toLowerCase().replace(/%20/g, ' '))
      .join('/');
