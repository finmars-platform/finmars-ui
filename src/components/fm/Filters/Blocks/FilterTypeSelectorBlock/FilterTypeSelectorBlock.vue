<template>
	...
	>
		<template #selection="{ value }">
			<span>{{ isFilterLinked ? 'Linked to' : (value as unknown[] as AvailableType[])[0].name }}</span>
		</template>
	</FmSelect>
</template>
...
<script lang="ts" setup>
	...
	import type {
		FmFilterTypeSelectorBlockProps,
		FmFilterTypeSelectorBlockEmits
	} from './types'
	...
	const availableFilterTypes = computed(() => {
		...
		return FILTER_CONDITIONS.reduce(
			(res, c) => {
