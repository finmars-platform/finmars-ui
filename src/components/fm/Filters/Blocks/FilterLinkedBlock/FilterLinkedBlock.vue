...
<script lang="ts" setup>
	...
	import type {
		FmFilterLinkedBlockProps,
		FmFilterLinkedBlockEmits
	} from './types'
	...
	const attrsEntityType = computed(() =>
		get(
			filterData.value,
			['options', 'use_from_above', 'attrs_entity_type'],
			''
		)
	)
	const attrKey = computed(() =>
		get(filterData.value, ['options', 'use_from_above', 'key'], '')
	)
	const displayAttrKey = computed(
		() => attrKey.value || props.locals!.linkedFieldPlaceholder
	)
	...
	async function getGroups() {
		try {
			...
			if (
				props.getSourceLinkedGroups &&
				typeof props.getSourceLinkedGroups === 'function'
			) {
	...
	async function getGroupAttributes() {
		try {
			...
			if (
				...
				attributes.value = await props.getSourceLinkedGroupAttributes(
					filterData.value!
				)
			}
		} finally {
