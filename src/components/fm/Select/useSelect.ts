...
export default function useSelect<T extends any & FmSelectOption, K extends string & keyof T>(
	...
	function onItemClick({ item, index }: { item: T; index: number }) {
		...
		emits('update:modelValue', props.returnObject ? item : item[props.valueKey as K]);
	...
	function handlePressingDownKey() {
		...
		if (activeOptionIndex.value >= 0 && activeOptionIndex.value < size(props.options) - 1) {
	...
	function handlePressingUpKey() {
		...
		if (activeOptionIndex.value > 0 && activeOptionIndex.value < size(props.options)) {
			activeOptionIndex.value -= 1;
		}
	}
