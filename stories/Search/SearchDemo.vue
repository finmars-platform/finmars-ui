<template>
  <div class="grid grid-cols-2 gap-16">
    <div>
      ...
      <FmSearch v-model="v1" v-on="eventHandlers" :items="items" label="Tu-tu" />
    </div>

    <div>
      <h3 class="text-[16px] mb-2">Base usage (disabled)</h3>

      <FmSearch v-on="eventHandlers" :items="items" disabled />
    </div>

    <div>
      <h3 class="text-[16px] mb-2">Base usage (multiple)</h3>

      <FmSearch v-model="v4" v-on="eventHandlers" multiple :items="items" />
    </div>

    <div>
      <h3 class="text-[16px] mb-2">Base usage (multiple and chips)</h3>

      <FmSearch v-model="v5" v-on="eventHandlers" multiple chips closable-chips :items="items" />
    </div>

    <div>
      <h3 class="text-[16px] mb-2">Use slot <i>item</i></h3>

      <FmSearch v-model="v2" v-on="eventHandlers" :items="items">
        <template #item="{ item }">
    ...
    <div>
      ...
      <FmSearch v-model="v3" v-on="eventHandlers" :items="items">
        <template #item="{ item }">
          <div class="flex items-center justify-start h-[48px] gap-x-1">
            <VIcon icon="mdi-star" />
            <span>{{ item.title }}</span>
